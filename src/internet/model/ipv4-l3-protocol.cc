                      ...
                      Ptr<Ipv4Route> route)
{
  ...
  uint8_t l4Ecn = tos & 0x3;
  uint8_t l4Dscp = tos & 0xFC;
  l4Dscp = l4Dscp ? l4Dscp : (socketTos & 0xFC);
  tos = l4Ecn | l4Dscp;
...
Ipv4L3Protocol::Fragments::AddFragment (Ptr<Packet> fragment, uint16_t fragmentOffset, bool moreFragment)
{
  ...
  m_fragments.insert (it, std::make_pair<Ptr<Packet>, uint16_t> (std::move(fragment), std::move(fragmentOffset)));
}
