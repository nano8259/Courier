...
inline void TcpSocketBase::LogSB(const char* str){
	return;
	static uint32_t cnt = 0;
	uint64_t tm = Simulator::Now().GetMilliSeconds();
	if(tm>300000 && tm<400000 && ++cnt<=10000)
		std::cout<<tm<<" "<<this<<" "<<str<<std::endl;
}

inline void TcpSocketBase::LogSB(uint32_t from, uint32_t to){
	return;
	static uint32_t cnt = 0;
	uint64_t tm = Simulator::Now().GetMilliSeconds();
	if(tm>300000 && tm<400000 && ++cnt<=10000)
		std::cout<<tm<<" "<<this<<"ack-seq from "<<from<<" to "<<to<<std::endl;
}
...
//                   MakeEnumChecker (CLOSED, "Closed"))
    ...
    .AddAttribute ("UserRTO",
                   ...
                   MakeTimeChecker())
    .AddAttribute ("AckDSCP",
                   "DSCP of ACK packets",
				   UintegerValue (Ipv4Header::DscpDefault),
				   MakeUintegerAccessor (&TcpSocketBase::m_ackDscp),
				   MakeUintegerChecker<uint8_t> (0, 255)) // by tbc
    .AddTraceSource ("RTO",
...
TcpSocketBase::TcpSocketBase (void)
  : m_dupAckCount (0),
    ...
    m_rWnd (0),
	m_lastRetxSeq(0), // by tbc
	m_retxCounter(0), // by tbc
	m_totalPackets(0),
	m_cePackets(0),
	m_lastUpdateTimeUs(Simulator::Now().GetMicroSeconds())
{
...
TcpSocketBase::TcpSocketBase (const TcpSocketBase& sock)
  : TcpSocket (sock),
    ...
    m_rWnd (sock.m_rWnd),
	m_ackDscp (sock.m_ackDscp), //by tbc
	m_lastRetxSeq(sock.m_lastRetxSeq), // by tbc
	m_retxCounter(sock.m_retxCounter),
	m_totalPackets(sock.m_totalPackets),
	m_cePackets(sock.m_cePackets),
	m_lastUpdateTimeUs(sock.m_lastUpdateTimeUs)
{
  NS_LOG_FUNCTION (this);
  ...
  SetRecvCallback (vPS);
  m_rc3Alpha = sock.m_rc3Alpha;
  m_rc3Beta = sock.m_rc3Beta;
  m_rc3Gamma = sock.m_rc3Gamma;
  m_rc3Probability = sock.m_rc3Probability;
...
TcpSocketBase::Send (Ptr<Packet> p, uint32_t flags)
{
  ...
  if (m_state == ESTABLISHED || m_state == SYN_SENT || m_state == CLOSE_WAIT)
    ...
    { // Connection not established yet
	  ...
	  //Log0("[Clock]  Time: connection error ");
                          ...
                          Ptr<Ipv4Interface> incomingInterface)
{
  ...
  DoForwardUp (packet, header, port, incomingInterface);
	/*c += clock()-t1;
	if(++n % 10000 == 0){
		std::cout<< "recv: "<<c / CLOCKS_PER_SEC << std::endl;
	}*/
}
                            ...
                            Ptr<Ipv4Interface> incomingInterface)
{
  ...
  // by tbc. record self machine and port
  this->SetRc3Machine( (header.GetDestination().Get()>>8)&0xff );
  this->SetRc3Port(m_endPoint->GetLocalPort ());

  // setting reducer socket in global table when being connected by sender. by tbc
  //if(m_endPoint->GetLocalPort () == 9) // reveiver end
 // GlobalProperty::m_receiverSocket[(header.GetSource().Get() >> 8) & 0xff][port] = this;
  ...
  if (m_EcnState & ECN_CONN)
    {
      ...
      /*if(header.GetEcn () == Ipv4Header::CE){
    	  NS_LOG_INFO ("CE packet received in L4");
    	  NS_LOG_INFO (" from " <<
                  m_endPoint->GetPeerAddress () <<
                  ":" << m_endPoint->GetPeerPort () <<
                  " to " << m_endPoint->GetLocalAddress () <<
...
TcpSocketBase::ReceivedAck (Ptr<Packet> packet, const TcpHeader& tcpHeader)
{
  ...
  if (0 == (tcpHeader.GetFlags () & TcpHeader::ACK))
    ...
    { // Case 3: New ACK, reset m_dupAckCount and update m_txBuffer
      ...
      m_dupAckCount = 0;
      if(m_retxCounter){
    	  LogSB(m_txBuffer.HeadSequence().GetValue(),tcpHeader.GetAckNumber().GetValue());
      }
      m_retxCounter = 0;//by tbc
    }
...
TcpSocketBase::SendEmptyPacket (uint16_t flags)
{
  ...
  uint8_t ECNbits = m_DCTCP ? Ipv4Header::ECT1 : 0; // modified by tbc
  
  bool isAck = (flags == TcpHeader::ACK);

  if(GlobalProperty::m_ack_priority) SetAckDscp(1);//added by lkx let ack and data go into different queue
  ...
  if (hasSyn)
    {
      ...
      if (m_cnCount == 0)
        ...
        { // Exponential backoff of connection time out
          ...
          m_cnCount--;
          if(m_cnCount<m_cnRetries-1) std::cout<<m_cnCount<<std::endl;
        }
    }
  if (m_endPoint != 0)
    {
      ...
      m_tcp->SendPacket (p, header, m_endPoint->GetLocalAddress (),
                         m_endPoint->GetPeerAddress (), m_ackDscp | ECNbits, m_boundnetdevice); // modified by tbc: set dscp
    }
  else
    {
...
TcpSocketBase::SendDataPacket (SequenceNumber32 seq, uint32_t maxSize, bool withAck)
{
  ...
  if (seq == m_nextTxSequence)
    {
      m_dataSentEvent = Simulator::ScheduleNow (&TcpSocketBase::NotifyDataSent, this, sz);
      LogSB("NotifyDataSent(pre)");
    }
  // Update highTxMark
...
TcpSocketBase::EstimateRtt (const TcpHeader& tcpHeader)
{
  ...
  if(nextRtt != 0)
  {
    ...
    // BY TBC
    /*uint64_t rtt = m_lastRtt.Get().GetMicroSeconds();
    if(rtt>RttEstimator::maxRTT)
    	RttEstimator::maxRTT = rtt;*/
  }
...
TcpSocketBase::DoRetransmit ()
{
  ...
  // add by tbc
  if(++m_retxCounter>=12){
	  std::cout<<std::endl<<"[TcpSocketBase::SendDataPacket] "<< this <<" connection reset"<<std::endl;
	  CloseAndNotify();
	  return ;
  }
...
TcpSocketBase::TimeWait ()
{
  ...
  m_timewaitEvent = Simulator::Schedule (MicroSeconds (2 * m_msl), // modified by tbc: seconds-> microseconds
...
void
TcpSocketBase::SetAckDscp (uint8_t ackDscp)
{
	m_ackDscp = ackDscp; // by tbc
}

uint8_t
TcpSocketBase::GetAckDscp (void) const
{
	return m_ackDscp; // by tbc
}

void TcpSocketBase::Output() const
{
	std::cout<<"m_state: "<<m_state<<"  "
			<<"m_errno: "<<m_errno<<"  "
			<<"m_closeNotified: "<<m_closeNotified<<"  "
			<<"m_closeOnEmpty: "<<m_closeOnEmpty<<"  "
			<<"m_shutdownSend: "<<m_shutdownSend<<"  "
			<<"m_shutdownRecv: "<<m_shutdownRecv<<"  "
			<<"m_connected: "<<m_connected<<std::endl;
}
