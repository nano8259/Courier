...
main (int argc, char *argv[])
{
	if(argc-1 < 6 || argc-1 > 7){
		cout<<"Usage: "<<endl;
		cout<<"udp-data-config <template-ini-file> <ini-section> <ini-item> <min-uint-value> <max-uint-value> <step> [<suffix>]"<<endl;
		return 0;
	}
	...
	fout<<"    ./waf --run \"udp-data-main "<<(out+".ini")<<" "<<("data/"+out+".txt")<<"\""<<std::endl;
	fout<<"}&"<<endl;
	fout<<"done"<<endl;
	fout.close();
