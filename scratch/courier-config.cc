 ...
 *      Author: tbc
 * 
 *  Modified by zczhang at Mar 14, 2021
 */
...
main (int argc, char *argv[])
{
	if(argc-1 < 6 || argc-1 > 7){
		cout<<"Usage: "<<endl;
		cout<<"courier-config <template-ini-file> <ini-section> <ini-item> <min-uint-value> <max-uint-value> <step> [<suffix>]"<<endl;
		return 0;
	}
	...
	string out_result = out.substr(7, out.length()); // remove "config/"
	fout<<"    ./waf --run \"courier "<<(out+".ini")<<" "<<("result/"+out_result+".log")<<"\" 2> result/"+out_result+".error"<<endl;
	fout<<"}&"<<endl;
	fout<<"done"<<endl;
	fout.close();
