...
protected:
  ...
  double m_rc3Beta;
  double m_rc3Gamma;
  double m_rc3Probability;
  uint32_t m_rc3Machine; // only used for sender
  uint32_t m_rc3Port; // only used for sender


public:
  // by tbc
  inline void CalcStates() {
	  m_rc3Beta = pow(m_rc3Alpha,m_rc3Gamma);
	  m_rc3Probability = m_rc3Alpha<m_rc3Beta ? (m_rc3Beta-m_rc3Alpha)/(1-m_rc3Alpha) : (m_rc3Alpha-m_rc3Beta)/m_rc3Alpha;
  }
  inline void SetRc3Gamma(double rc3Gamma) { m_rc3Gamma = rc3Gamma; CalcStates(); }
  inline void SetRc3Alpha(double rc3Alpha) { m_rc3Alpha = rc3Alpha; CalcStates(); /*std::cout<<this<<"\t"<<rc3Alpha<<"\t"<<m_rc3Beta<<"\t"<<m_rc3Gamma<<"\t"<<m_rc3Probability<<std::endl;*/}

  inline uint32_t GetRc3Machine() const { return m_rc3Machine; }
  inline uint32_t GetRc3Port() const { return m_rc3Port; }
  inline void SetRc3Machine(uint32_t machine) { m_rc3Machine = machine; }
  inline void SetRc3Port(uint32_t port) { m_rc3Port = port; }
