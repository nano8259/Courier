...
public:
  ...
  // DSCP functions
  virtual void SetAckDscp (uint8_t ackDscp); // by tbc
  virtual uint8_t GetAckDscp (void) const;  // by tbc
  virtual void Output() const; //by tbc
  inline void LogSB(const char* str);
  inline void LogSB(uint32_t from, uint32_t to);
...
protected:
  ...
  // DSCP ACK
  uint8_t               m_ackDscp; // default dscp value of ack packets, only in ipv4. by tbc

  // close flags  // by tbc
  //uint32_t m_lastRemainingSizeClosing; // when closing, the remaining size in tx_buffer in last simulation steps. by tbc
  uint32_t m_lastRetxSeq; //
  uint32_t m_retxCounter; // we record the number of retries when closing to avoid infinite retransmission. by tbc
  EventId m_dataSentEvent;

  // alpha update, by tbc
  uint32_t m_totalPackets;
  uint32_t m_cePackets;
  uint64_t m_lastUpdateTimeUs;
